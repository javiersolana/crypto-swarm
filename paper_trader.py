"""
Paper Trader - Simulated trading with TP/SL/Trailing Stop (v8.8 Sweet Spot)

v7.0 Moonbag Strategy: TP1 partial close + trailing on moonbag.
v8.5 Break-even SL: At +N% profit, SL moves to entry price.
v8.8 Sweet Spot:
  - Tiered position sizing: open_trade() accepts optional amount_sol
  - SL=-12% (was -18%), TP1=+40% (was +80%), sell 60% (was 50%)
  - Breakeven at +20% (was +15%), moonbag trailing 15% (was 20%)
  - Max 5 open trades (was 20) — realistic for 0.5 SOL account
  - Slippage 5% (was 3%) — matches real DEX execution data
...
class PaperTrader:
    """SOL-based paper trading with Moonbag exit strategy (v8.8 Sweet Spot).

    v8.8 Strategy:
      - Tiered sizing: 0.05/0.025/0.015 SOL based on confidence
      - TP1 at +40%: sell 60% → secure capital (achievable target)
      - Trailing Stop 15% on remaining 40% (moonbag)
      - SL at -12% → real exit ~-15% with slippage (cut losses fast)
      - Break-even SL at +20% profit (above noise zone)
    ...
    def open_trade(self, token: dict, amount_sol: float = None) -> dict | None:
        """Open a paper trade for an alerted token.

        Args:
            token: Token dict with price_usd, address, chain, etc.
            amount_sol: Optional position size. If None, uses config default.
                        v8.8: Tiered sizing passes this from alpha_monitor.
        ...
        with self._lock:
            ...
            # v8.8: Use provided tier amount or fall back to config default
            ...
            trade = {
                ...
                "breakeven_activated": False,  # v8.5: SL moved to entry at +15%
    ...
    def close_trade(self, trade_id: str, exit_price: float, reason: str) -> dict | None:
        ...
        with self._lock:
            ...
            trade.update({
                ...
                "result": result,
                "pnl_sol": float(round(total_pnl_sol, 6)),
                "pnl_pct": float(round(pnl_pct, 6)),             # v8.2: 6 decimal precision
                "final_change_pct": float(round(pnl_pct, 6)),    # v8.2: 6 decimal precision for reports
                "fee_sol": float(round(fee_sol, 6)),
                "pnl_net_sol": float(round(total_pnl_sol, 6)),
                "pnl_sol_remaining": float(round(pnl_sol_remaining, 6)),
                "exit_reason": reason,
                "status": "closed",
            })
    ...
    def update_prices(self, price_map: dict):
        ...
        with self._lock:
            ...
            for trade in self._data["open_trades"]:
                ...
                # v8.5: Break-even SL at +15% profit (before TP1)
                if not trade.get("breakeven_activated") and not trade.get("tp1_hit"):
                    breakeven_pct = getattr(config, 'PAPER_BREAKEVEN_PCT', 15)
                    pnl_pct = ((current - trade["entry_price"]) / trade["entry_price"]) * 100
                    if pnl_pct >= breakeven_pct:
                        trade["breakeven_activated"] = True
                        trade["stop_loss"] = trade["entry_price"]
                        log.info(f"BREAKEVEN SL: {trade['token_name']} hit +{pnl_pct:.1f}% "
                                 f"— SL moved to entry ${trade['entry_price']:.8f}")
    ...
    def check_exits(self, price_map: dict) -> list[dict]:
        ...
        with self._lock:
            for trade in list(self._data["open_trades"]):
                ...
                # v8.5: Check break-even activation inline (same logic as update_prices)
                if not trade.get("breakeven_activated") and not trade.get("tp1_hit"):
                    breakeven_pct = getattr(config, 'PAPER_BREAKEVEN_PCT', 15)
                    if pnl_pct >= breakeven_pct:
                        trade["breakeven_activated"] = True
                ...
                if current <= trade["stop_loss"]:
                    ...
                    elif trade.get("breakeven_activated"):
    ...
    def format_open_message(self, trade: dict) -> str:
        ...
        return (
            ...
            f"Break-even SL at: +{getattr(config, 'PAPER_BREAKEVEN_PCT', 15)}%\n"
